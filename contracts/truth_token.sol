...
contract TRUTHToken is Context, AccessControlEnumerable, ERC20Votes {
    bytes32 public constant MINTER_ROLE = keccak256("MINTER_ROLE");
    bytes32 public constant BURNER_ROLE = keccak256("BURNER_ROLE");

    modifier onlyHasRole(bytes32 _role) {
        require(hasRole(_role, _msgSender()), "TRUTHToken.onlyHasRole: msg.sender does not have role");
        _;
    }
    ...
    function _transfer(address _from, address _to, uint256 _amount) internal override {
        require(_to != address(this), "TRUTHToken._transfer: transfer to self not allowed");
